...
####################################################################
# Sources
# This version keeps the original file names, but the m2/m3 and subtrial
# rules are modified inside those files.
####################################################################
...
lambda1    <- 0.16; # modified lower tox boundary
lambda2    <- 0.33; # modified upper tox boundary
eta1       <- 0.46; # modified efficacy boundary
...
for (kk in 1:length(c.sc)){
  ...
  saveWorkbook(wb, paste("../../output/BOINTEC.new.summary_", out.dl, "_", out.util, ".xlsx"), 
               overwrite = TRUE)
}
